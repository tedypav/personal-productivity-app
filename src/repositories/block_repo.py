...
class BlockRepo:
    ...
    def create(block: ContentBlock) -> int:
        ...
        cursor = conn.execute(
            "INSERT INTO content_blocks (page_id, block_type, content_markdown, sort_order) VALUES (?, ?, ?, ?)",
            (block.page_id, block.block_type, block.content_markdown, max_order)
        )
        conn.commit()
    ...
    def update(block: ContentBlock):
        conn = get_connection()
        conn.execute(
            "UPDATE content_blocks SET content_markdown=?, block_type=?, sort_order=? WHERE id=?",
            (block.content_markdown, block.block_type, block.sort_order, block.id)
        )
        conn.commit()
        conn.close()
