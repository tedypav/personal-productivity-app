...
class TestPageRepoCreate:
    def test_creates_page_and_returns_id(self, repo, db_init):
        page_id = repo.create(Page(title="Test Page"))
        assert page_id is not None
        assert page_id > 0

    def test_auto_computes_sort_order_when_zero(self, repo, db_init):
...
class TestPageRepoRead:
    def test_get_by_id_returns_page(self, repo, db_init):
        p_id = repo.create(Page(title="Find Me"))
        found = repo.get_by_id(p_id)
        assert found is not None
        assert found.title == "Find Me"

    def test_get_by_id_returns_none_for_nonexistent(self, repo, db_init):
        assert repo.get_by_id(99999) is None

    def test_get_all_returns_all_pages(self, repo, db_init):
        repo.create(Page(title="A"))
        repo.create(Page(title="B"))
        all_pages = repo.get_all()
        assert len(all_pages) == 2

    def test_get_all_sorted_by_sort_order(self, repo, db_init):
    ...
    def test_get_children_root(self, repo, db_init):
    ...
    def test_get_children_specific_parent(self, repo, db_init):
...
class TestPageRepoUpdate:
    def test_updates_page(self, repo, db_init):
    ...
    def test_updates_parent_id(self, repo, db_init):
        p1_id = repo.create(Page(title="P1"))
        p2_id = repo.create(Page(title="P2"))
    ...
    def test_update_refreshes_updated_at(self, repo, db_init):
...
class TestPageRepoDelete:
    def test_deletes_page(self, repo, db_init):
        p_id = repo.create(Page(title="Delete Me"))
        repo.delete(p_id)
        assert repo.get_by_id(p_id) is None

    def test_delete_cascades_to_children(self, repo, db_init):
        parent_id = repo.create(Page(title="Parent"))
...
class TestPageRepoReorder:
    def test_reorder_updates_sort_order(self, repo, db_init):
        p_id = repo.create(Page(title="P", sort_order=1))
        repo.reorder(p_id, 5, None)
        page = repo.get_by_id(p_id)
        assert page.sort_order == 5

    def test_reorder_updates_parent(self, repo, db_init):
        p1_id = repo.create(Page(title="P1"))
        p2_id = repo.create(Page(title="P2"))
