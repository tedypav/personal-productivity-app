...
from src.undo_manager import (
    UndoManager, capture_page_tree, UNDO_DURATION,
    _page_dict, _block_dict, _task_dict,
    _capture_children,
)
...
class TestUndoManager:
    ...
    def test_pop_returns_most_recent(self, undo_mgr, db_init):
        ...
        undo_mgr.push({"type": "block", "block": {"id": bid, "page_id": pid, "block_type": "text", "content_markdown": "", "sort_order": 0}, "tasks": []})
        undo_mgr.push({"type": "task", "task": {"id": 99, "content_block_id": bid, "text": "t"}})
        result = undo_mgr.pop()
    ...
    def test_prune_removes_expired(self, undo_mgr):
        ...
        undo_mgr._actions[0]["timestamp"] = datetime.now() - UNDO_DURATION - timedelta(seconds=1)
    ...
    def test_pop_restores_page(self, undo_mgr, db_init):
        ...
        undo_mgr.push({
            ...
            "blocks": [_block_dict(ContentBlock(id=bid, page_id=pid, block_type="text"))],
    ...
    def test_pop_restores_block(self, undo_mgr, db_init):
        ...
        undo_mgr.push({
            "type": "block",
            "block": _block_dict(ContentBlock(id=bid, page_id=pid, block_type="text")),
            "tasks": [],
        })
        undo_mgr.pop()
...
class TestCapturePageTree:
    ...
    def test_captures_children(self, db_init):
        ...
        child = PageRepo.create(Page(title="child", parent_id=parent))
...
class TestCaptureChildren:
    ...
    def test_captures_nested_children(self, db_init):
        ...
        p3 = PageRepo.create(Page(title="p3", parent_id=p2))
...
class TestDictHelpers:
    def test_page_dict(self):
        p = Page(id=1, title="T", parent_id=2, sort_order=3, page_type="folder",
    ...
    def test_block_dict(self):
        b = ContentBlock(id=1, page_id=2, block_type="table",
                         content_markdown="|a|", sort_order=5)
        d = _block_dict(b)
        assert d["id"] == 1
        assert d["page_id"] == 2
    ...
    def test_task_dict(self):
        t = Task(id=1, content_block_id=2, text="hello",
                 is_checked=True, recurrence_type="daily",
                 due_date="2024-06-15", parent_task_id=3, sort_order=4)
        d = _task_dict(t)
        assert d["id"] == 1
